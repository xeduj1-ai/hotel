...
function createReservation(guestNameArg, typeArg, checkInArg, checkOutArg, rateArg) {
  // 1. Resolve arguments with fallback to DOM (Legacy support)
  let guestName = guestNameArg;
  let type = typeArg;
  let checkIn = checkInArg;
  let checkOut = checkOutArg;
  let rate = rateArg;
  ...
  if (!type) type = document.getElementById('newResType')?.value;
  if (!checkIn) checkIn = document.getElementById('newResIn')?.value;
  if (!checkOut) checkOut = document.getElementById('newResOut')?.value;
  if (!rate) rate = parseFloat(document.getElementById('newResRate')?.value);
  ...
  // 2. Validation
  if (!guestName || !type || !checkIn || !checkOut) {
    return showToast('Por favor completa todos los campos (Huésped, Tipo, Fechas)', 'warning');
  }

  // 3. Rate fallback logic
  if (!rate) {
  ...
  if (!finalRoom) {
    ...
    return showToast(`No hay habitaciones ${type} limpias disponibles.`, 'warning');
  }
  ...
  if (document.getElementById('selectedRoomInfo')) {
  ...
  // Close modal if open (try both IDs)
  closeModal('resModal');
  // If using generic modal helper
  ...
  renderReservations();
  addNotification(`Nueva reserva creada: ${guestName} (Hab ${finalRoom.id})`);
...
function calculateReservationFinancials(res) {
  ...
  // USER RULE: No IVA (16%) on the portion paid in Divisa.
  // IVA applies only to the subtotal NOT covered by Divisa payments.
  // Ensure we don't credit more divisa than the subtotal for tax purposes (clamping at 0)
  const subtotalSubjectToIVA = Math.max(0, subtotal - divisaPaymentsTotal);
  const ivaRate = 0.16;
  const iva = subtotalSubjectToIVA * ivaRate;

  const igtfRate = 0.03;
  // SENIAT Refinement: IGTF applies ONLY to the portion paid in divisa
  const igtf = divisaPaymentsTotal * igtfRate;
  const grandTotal = subtotal + iva + igtf;
  ...
  return {
    ...
    totalRetained,
    balance,
    divisaPaymentsTotal // Exported for smart payoff calculation
  };
}
...
function processBillPayment(resId, isCheckout = false) {
  ...
  document.getElementById('folioModalBody').innerHTML = `
    <div style="padding:24px">
        ...
        <div id="paymentSummaryContent" style="background:var(--bg-surface);padding:16px;border-radius:12px;margin-bottom:24px;text-align:center;border:2px dashed var(--gold)">
            <div class="text-muted text-xs">TOTAL A PAGAR (CON IVA)</div>
            <div style="font-size:32px;font-weight:800;color:var(--primary)">$${fin.balance.toFixed(2)}</div>
            <div style="font-size:14px;color:var(--text-secondary);margin-top:4px">
                ≈ Bs. ${(fin.balance * (DB.settings.exchangeRate || 36.50)).toFixed(2)}
            </div>
            ${fin.iva > 0 ? `<div style="font-size:11px; color:var(--text-secondary); margin-top:8px">IVA Total: $${fin.iva.toFixed(2)} (16%)</div>` : ''}
            ${fin.totalRetained > 0 ? `<div style="font-size:11px; color:var(--success); font-weight:700">Retenido IVA: -$${fin.totalRetained.toFixed(2)}</div>` : ''}
            
            <button class="btn btn-outline btn-xs mt-8" onclick="recordWithholdingIVA('${resId}', ${isCheckout})" style="margin-top:12px">
...
function toggleDivisaSelection(resId, isCheckout) {
  ...
  if (mainGrid.classList.contains('hidden')) {
    ...
    summaryBox.innerHTML = `
        <div class="text-xs" style="color:var(--text-secondary); font-weight:700">TOTAL A PAGAR (CON IVA)</div>
        <div style="font-size:32px;font-weight:800;color:var(--primary)">$${fin.balance.toFixed(2)}</div>
        <div style="font-size:14px;color:var(--text-secondary);margin-top:4px">
            ≈ Bs. ${(fin.balance * (DB.settings.exchangeRate || 36.50)).toFixed(2)}
        </div>
                ...
                <div style="width:40%">
                    <table style="width:100%; border-collapse:collapse">
                        ...
                        <tr>
                            ...
                            <td style="padding:8px 0; text-align:right; font-size:13px; font-weight:600; color:#1e293b">$${fin.iva.toFixed(2)}</td>
                        </tr>` : ''}
                        ${fin.igtf > 0 ? `
                        <tr>
...
function preparePaymentAmount(resId, methodId, isCheckout) {
  ...
  let amountToPay = fin.balance;

  if (isDivisa && fin.balance > 0) {
    // Calculate amount needed to settle debt using Divisa (Exempt from IVA, subject to 3% IGTF)
    const S = fin.subtotal;
    const Pb = fin.totalPaid - fin.divisaPaymentsTotal; // Paid in Bs
    const Pd = fin.divisaPaymentsTotal; // Paid in Divisa

    // Target Divisa needed to cover (S per IVA rules) - Pb
    // Formula derived from: Balance = 0 => 1.16*S - Pb - 1.13*TotalDivisa = 0
    let targetDi = (1.16 * S - Pb) / 1.13;

    // Check Regime 2: If Divisa covers Subtotal entirely, IVA becomes 0.
    // Validity: TotalDivisa >= S
    if (targetDi > S) {
      // Regime 2 Formula: TotalDivisa = (S - Pb) / 0.97
      targetDi = (S - Pb) / 0.97;
    }

    const due = Math.max(0, targetDi - Pd);
    // Use the smarter amount, but don't exceed the generic balance (safety, though Divisa should always be lower)
    amountToPay = Math.min(fin.balance, due);

    // Safety: If due is very close to balance, use balance simply? 
    // No, strictly use due.
    amountToPay = due;
  }
  ...
  const amountUSD = amountToPay;
  const amountBS = amountToPay * rate;
         ...
         <div class="form-group">
            ...
            <div style="display:flex; gap:10px">
                <input type="number" id="paymentAmount" class="form-input" style="font-size:24px; font-weight:bold; padding:12px; background:rgba(255,255,255,0.05); color:var(--text-main)" 
                       value="${(isLocalMethod ? amountBS : amountUSD).toFixed(2)}" step="0.01">
            </div>
            ${isLocalMethod ? `<div class="text-xs mt-4" style="color:var(--text-secondary)">Equivalente a $${amountUSD.toFixed(2)}</div>` :
      `<div class="text-xs mt-4" style="color:var(--text-secondary)">Equivalente a Bs. ${amountBS.toFixed(2)}</div>`
    }
             <div class="text-xs mt-4" style="color:var(--text-secondary)">
               ${isDivisa ?
      `ℹ️ <strong>Optimización Fiscal:</strong> Monto ajustado para exención de IVA. (Incluye 3% IGTF)` :
      `ℹ️ Incluye IVA (16%).`}
             </div>
         </div>
...
function openNewQuoteModal() {
  ...
  document.getElementById('qPaxCount').value = '1';
  ...
  // Fill room types
  const typeSelect = document.getElementById('qRoomType');
  typeSelect.innerHTML = DB.settings.roomTypes.map(t => `<option value="${t.id}">${t.label} ($${t.price})</option>`).join('');

  openModal('quoteModal');
}

function saveQuotation() {
  const agencyName = document.getElementById('qAgencyName').value;
  const agencyRif = document.getElementById('qAgencyRif').value;
  const agencyPhone = document.getElementById('qAgencyPhone').value;
  const roomTypeId = document.getElementById('qRoomType').value;
  ...
  if (!agencyName || !agencyRif || !agencyPhone || !rate || !checkIn || !checkOut) {
    return showToast('Por favor complete los campos obligatorios (*)', 'warning');
  }

  const roomType = DB.settings.roomTypes.find(t => t.id === roomTypeId);
...
function renderQuotations() {
  ...
  const filtered = DB.quotations.filter(q => {
    const matchesSearch = q.locator.toLowerCase().includes(searchTerm) ||
      q.agencyName.toLowerCase().includes(searchTerm);
  ...
  tableBody.innerHTML = filtered.sort((a, b) => b.id - a.id).map(q => `
    <tr>
      ...
      <td>${q.agencyName}<br><small class="text-muted">${q.agencyRif}</small></td>
      <td>${q.paxCount} Pax</td>
      <td>${q.roomTypeName}</td>
      <td>${q.checkIn}</td>
    ...
    <div style="font-family:Arial, sans-serif; padding:40px; color:#333; line-height:1.4">
      <div style="display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:20px; border-bottom:2px solid #005aab; padding-bottom:10px">
        ...
        <div style="text-align:center; flex:1">
          <h2 style="margin:0; color:#005aab">COTIZACIÓN NUEVA TOLEDO SUITES & HOTEL</h2>
        </div>
      </div>
      ...
      <div style="display:grid; grid-template-columns: 1.5fr 1fr; border:1px solid #333; margin-bottom:10px">
        <div style="padding:10px; border-right:1px solid #333">
          <table style="width:100%; font-size:12px">
            <tr><td width="30%"><strong>Nombre:</strong></td><td>${q.agencyName}</td></tr>
            <tr><td><strong>Tarifa:</strong></td><td>TARIFA AGENCIA</td></tr>
            <tr><td><strong>RIF/C.I:</strong></td><td>${q.agencyRif}</td></tr>
            <tr><td><strong>E-mail:</strong></td><td>${q.agencyEmail || 'N/A'}</td></tr>
            <tr><td><strong>Teléfonos:</strong></td><td>${q.agencyPhone}</td></tr>
            <tr><td><strong>Nº de pax:</strong></td><td>${q.paxCount}</td></tr>
            <tr><td><strong>Pasajeros:</strong></td><td>${q.passengers || 'N/A'}</td></tr>
          </table>
        </div>
        <div style="padding:10px">
          <table style="width:100%; font-size:12px">
            ...
            <tr><td><strong>Cotización realizada:</strong></td><td>${q.date}</td></tr>
          </table>
        </div>
      </div>
      ...
      <p style="font-size:11px; margin-bottom:10px">Estimado/a: Reciba un saludo muy cordial del equipo comercial del Hotel Nueva Toledo Suites, en atención a su solicitud de cotización, le presento la siguiente propuesta.</p>

      <table style="width:100%; border-collapse:collapse; font-size:11px; margin-bottom:10px">
        <thead>
          <tr style="background:#eee; border:1px solid #333">
            <th style="padding:5px; border:1px solid #333">Cantidad</th>
            ...
            <th style="padding:5px; border:1px solid #333">Tarifa sin IVA</th>
            <th style="padding:5px; border:1px solid #333">Total sin IVA</th>
          </tr>
        </thead>
        <tbody>
          <tr style="text-align:center">
            <td style="padding:5px; border:1px solid #333">1</td>
            <td style="padding:5px; border:1px solid #333">${q.roomTypeName.toUpperCase()}<br>${q.notes.toUpperCase()}</td>
            ...
            <td style="padding:5px; border:1px solid #333">$ ${q.rate.toFixed(2)}</td>
            <td style="padding:5px; border:1px solid #333">$ ${q.subtotal.toFixed(2)}</td>
          </tr>
        </tbody>
      </table>

      <div style="background:#005aab; color:white; text-align:center; padding:3px; font-size:10px; font-weight:bold; margin-bottom:10px">
        TARIFA PAGADA EN BOLIVARES SEGÚN LA TASA DE CAMBIO OFICIAL DEL BANCO CENTRAL DE VENEZUELA DEL DIA.
      </div>

      <div style="background:#005aab; color:white; padding:5px; font-weight:bold; font-size:12px">ENTIDADES BANCARIAS - FORMA DE PAGO: DEPÓSITOS Y/O TRANSFERENCIAS</div>
      <div style="border:1px solid #333; padding:10px; display:flex; justify-content:space-between">
        ...
        <div style="width:35%; font-size:12px; font-weight:bold">
          <div style="display:flex; justify-content:space-between"><span>TOTAL SIN IVA</span> <span>$ ${q.subtotal.toFixed(2)}</span></div>
          <div style="display:flex; justify-content:space-between; margin-top:10px"><span>16%</span> <span>$ ${q.iva.toFixed(2)}</span></div>
          <div style="display:flex; justify-content:space-between; margin-top:10px; border-top:2px solid #333; padding-top:5px"><span>TOTAL GENERAL</span> <span>$ ${q.total.toFixed(2)}</span></div>
        </div>
      </div>

      <div style="margin-top:10px; font-size:9px; border:1px solid #333; padding:10px">
        <div style="text-align:center; font-weight:bold; margin-bottom:5px">CONDICIONES Y/O NORMATIVAS ESTABLECIDAS PARA LA CONTRATACIÓN DE NUESTROS SERVICIOS:</div>
        1.- TARIFAS NO REEMBOLSABLES POR CAMBIOS Y/O ANULACIÓN DE RESERVAS.<br>
        2.- Tarifas sujetas a cambio sin previo aviso.<br>
        3.- La forma de pago es a través de depósitos bancarios, transferencias, tarjeta de débito o crédito.<br>
        4.- La empresa no acepta bajo ningún concepto pagos en efectivo ante ninguno de sus representantes y/o terceros. De igual manera no son válidos los depósitos, transferencias y cheques cuyo beneficiario sea distinto a Promociones Nueva Toledo, C.A.<br>
        5.- En caso de contratar algún servicio de comida con la empresa, una vez ofertado y pagado, este no tiene anulación alguna, sumado a este, no se realiza reintegro alguno en efectivo, transferencias y/o saldos internos para futuros consumos.<br>
        6.- Para reservar debe consultar la disponibilidad de la oferta presupuestada.<br>
        7.- Enviar físico de la operación bancaria vía correo o whatsapp para validar su reserva y enviar localizador.<br>
        8.- Hora de entrada: 03:00 pm. Hora de salida: 01:00 pm. Si desea entrar antes de la hora o salir después de la hora, con una diferencia de 5 horas, debe consultar la disponibilidad con anticipación y prepagar el 50% de la tarifa aplicada.<br>
        9.- Al llegar a las instalaciones se le solicitará depósitos en efectivo en garantía por control A/A y TV, toallas de piscina y llaves adicionales. El mismo se le regresará al momento de retirarse de la empresa.<br>
        ...
        11.- El área del estacionamiento es libre para uso del huésped, público, cliente por puerta, eventos y personal empleado del Hotel, en caso de algún accidente durante la estadía es responsabilidad ÚNICA Y ABSOLUTA de los involucrados.
      </div>
...
function convertQuotationToReservation(quoteId) {
  ...
  if (confirm(`¿Convertir cotización ${q.locator} a reserva confirmada?`)) {
    // 1. Create Guest if doesn't exist (searching by name or RIF)
    ...
    const newRes = {
      ...
      guestId: guest.id,
      roomId: null, // To be assigned manually by reception
      checkIn: q.checkIn,
      checkOut: q.checkOut,
      status: 'confirmada',
      channel: 'Corporativo',
      ...
      folio: [{ name: `Estancia (${q.nights} noches) - Cot. ${q.locator}`, price: q.total, time: 'Init' }]
    };
    ...
    // Navigate to reservations to show the result
    navigate('reservations');
  }
}
